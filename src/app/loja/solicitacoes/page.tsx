import Link from "next/link";
import { getCurrentUser } from "@/lib/auth/session";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/db/prisma";
import { formatEnumLabel } from "@/lib/formatters/display";
import { EMPRESAS } from "@/config/domains";
import { OperationalRequestsPanel } from "@/components/loja/operational-requests-panel";

type PageProps = {
  searchParams: Promise<{
    empresa?: string;
    status?: string;
  }>;
};

type EmpresaValue = (typeof EMPRESAS)[number];

type StatusOperacional =
  | "EM_ABERTO"
  | "ASSISTENCIA_ENVIADA"
  | "ASSISTENCIA_A_CAMINHO"
  | "ASSISTENCIA_ENTREGUE"
  | "COLETA_SOLICITADA"
  | "COLETA_FEITA"
  | "DEVOLUCAO_SOLICITADA"
  | "DEVOLUCAO_A_CAMINHO"
  | "DEVOLUCAO_REALIZADA"
  | "REEMBOLSO_PENDENTE"
  | "REEMBOLSO_REALIZADO"
  | "AGUARDANDO_ATENDENTE"
  | "CONCLUIDA";

type OperationalRequestWithTicket = {
  id: string;
  empresa: string;
  ticketId: string;
  tipoAcao: string;
  status: StatusOperacional;
  prazoOperacional: Date | null;
  updatedAt: Date;
  comentarioLoja: string | null;
  comentarioAtendente: string | null;
  codigoRastreio: string | null;
  valorReembolso: unknown;
  valorColetaEnvioPecas: unknown;
  ticket: {
    nomeCliente: string;
    numeroVenda: string;
    linkPedido: string | null;
    anexoUrl: string | null;
    anexoNome: string | null;
    anexoPath: string | null;
  };
};

const statusOptions: StatusOperacional[] = [
  "EM_ABERTO",
  "ASSISTENCIA_ENVIADA",
  "ASSISTENCIA_A_CAMINHO",
  "ASSISTENCIA_ENTREGUE",
  "COLETA_SOLICITADA",
  "COLETA_FEITA",
  "DEVOLUCAO_SOLICITADA",
  "DEVOLUCAO_A_CAMINHO",
  "DEVOLUCAO_REALIZADA",
  "REEMBOLSO_PENDENTE",
  "REEMBOLSO_REALIZADO",
  "AGUARDANDO_ATENDENTE",
  "CONCLUIDA"
];

const statusLabel: Record<string, string> = {
  aberto: "Em aberto",
  assistenciaEnviada: "Assistência enviada",
  assistenciaCaminho: "Assistência a caminho",
  assistenciaEntregue: "Assistência entregue",
  coletaSolicitada: "Coleta solicitada",
  coletaFeita: "Coleta feita",
  devolucaoSolicitada: "Devolução solicitada",
  devolucaoCaminho: "Devolução a caminho",
  devolucaoRealizada: "Devolução realizada",
  reembolsoPendente: "Reembolso pendente",
  reembolsoRealizado: "Reembolso realizado",
  aguardandoAtendente: "Aguardando atendente",
  concluidas: "Concluídas",
  atrasadas: "Atrasadas"
};

function isEmpresa(value: string | undefined): value is EmpresaValue {
  return Boolean(value && EMPRESAS.includes(value as EmpresaValue));
}

function isStatusOperacional(value: string | undefined): value is StatusOperacional {
  return Boolean(value && statusOptions.includes(value as StatusOperacional));
}

function toNumber(value: unknown): number {
  if (value === null || value === undefined || value === "") return 0;

  if (typeof value === "number") {
    return Number.isFinite(value) ? value : 0;
  }

  if (typeof value === "string") {
    const parsed = Number(value.replace(",", "."));
    return Number.isFinite(parsed) ? parsed : 0;
  }

  if (typeof value === "object" && value !== null && "toNumber" in value) {
    const parsed = (value as { toNumber: () => number }).toNumber();
    return Number.isFinite(parsed) ? parsed : 0;
  }

  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
}

export default async function LojaSolicitacoesPage({ searchParams }: PageProps) {
  const user = await getCurrentUser();

  if (user.perfil !== "LOJA" && user.perfil !== "ADMIN") {
    redirect("/dashboard");
  }

  const params = await searchParams;

  const empresaFiltro = isEmpresa(params.empresa) ? params.empresa : undefined;
  const statusFiltro = isStatusOperacional(params.status) ? params.status : undefined;

  const baseWhere =
    user.perfil === "LOJA"
      ? { empresa: user.empresaVinculada ?? undefined }
      : empresaFiltro
        ? { empresa: empresaFiltro }
        : {};

  const where = {
    ...baseWhere,
    ...(statusFiltro ? { status: statusFiltro } : {})
  };

  const data = (await prisma.operationalRequest.findMany({
    where,
    include: {
      ticket: {
        select: {
          nomeCliente: true,
          numeroVenda: true,
          linkPedido: true,
          anexoUrl: true,
          anexoNome: true,
          anexoPath: true
        }
      }
    },
    orderBy: {
      updatedAt: "desc"
    }
  })) as unknown as OperationalRequestWithTicket[];

  const stats = {
    aberto: data.filter((d) => d.status === "EM_ABERTO").length,
    assistenciaEnviada: data.filter((d) => d.status === "ASSISTENCIA_ENVIADA").length,
    assistenciaCaminho: data.filter((d) => d.status === "ASSISTENCIA_A_CAMINHO").length,
    assistenciaEntregue: data.filter((d) => d.status === "ASSISTENCIA_ENTREGUE").length,
    coletaSolicitada: data.filter((d) => d.status === "COLETA_SOLICITADA").length,
    coletaFeita: data.filter((d) => d.status === "COLETA_FEITA").length,
    devolucaoSolicitada: data.filter((d) => d.status === "DEVOLUCAO_SOLICITADA").length,
    devolucaoCaminho: data.filter((d) => d.status === "DEVOLUCAO_A_CAMINHO").length,
    devolucaoRealizada: data.filter((d) => d.status === "DEVOLUCAO_REALIZADA").length,
    reembolsoPendente: data.filter((d) => d.status === "REEMBOLSO_PENDENTE").length,
    reembolsoRealizado: data.filter((d) => d.status === "REEMBOLSO_REALIZADO").length,
    aguardandoAtendente: data.filter((d) => d.status === "AGUARDANDO_ATENDENTE").length,
    concluidas: data.filter((d) => d.status === "CONCLUIDA").length,
    atrasadas: data.filter((d) => d.prazoOperacional && d.prazoOperacional < new Date() && d.status !== "CONCLUIDA").length
  };

  return (
    <section className="page">
      <h1>Solicitações da Loja</h1>

      <div className="panel" style={{ marginBottom: 12 }}>
        <form
          action="/loja/solicitacoes"
          method="get"
          style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end" }}
        >
          {user.perfil === "ADMIN" ? (
            <label style={{ minWidth: 220 }}>
              Empresa
              <select name="empresa" defaultValue={empresaFiltro ?? ""}>
                <option value="">Todas as empresas</option>
                {EMPRESAS.map((emp) => (
                  <option key={emp} value={emp}>
                    {formatEnumLabel(emp)}
                  </option>
                ))}
              </select>
            </label>
          ) : null}

          <label style={{ minWidth: 260 }}>
            Status operacional
            <select name="status" defaultValue={statusFiltro ?? ""}>
              <option value="">Todos os status</option>
              {statusOptions.map((status) => (
                <option key={status} value={status}>
                  {formatEnumLabel(status)}
                </option>
              ))}
            </select>
          </label>

          <button className="btn btn-primary" type="submit">
            Filtrar
          </button>

          <Link className="btn btn-secondary" href="/loja/solicitacoes">
            Limpar
          </Link>
        </form>
      </div>

      <div className="grid" style={{ gridTemplateColumns: "repeat(4,minmax(0,1fr))" }}>
        {Object.entries(stats).map(([key, value]) => (
          <article className="card" key={key}>
            <strong>{statusLabel[key] ?? key}</strong>
            <div style={{ fontSize: 24 }}>{value}</div>
          </article>
        ))}
      </div>

      <OperationalRequestsPanel
        perfil={user.perfil}
        data={data.map((row) => ({
          id: row.id,
          empresa: row.empresa,
          ticketId: row.ticketId,
          tipoAcao: row.tipoAcao,
          status: row.status,
          prazoOperacional: row.prazoOperacional?.toISOString() ?? null,
          updatedAt: row.updatedAt.toISOString(),
          comentarioLoja: row.comentarioLoja ?? null,
          comentarioAtendente: row.comentarioAtendente ?? null,
          codigoRastreio: row.codigoRastreio ?? null,
          valorReembolso: toNumber(row.valorReembolso),
          valorColetaEnvioPecas: toNumber(row.valorColetaEnvioPecas),
          ticket: {
            nomeCliente: row.ticket.nomeCliente,
            numeroVenda: row.ticket.numeroVenda,
            linkPedido: row.ticket.linkPedido ?? null
          },
          anexo: row.ticket.anexoUrl
            ? {
                fileUrl: row.ticket.anexoUrl,
                fileName: row.ticket.anexoNome ?? "Anexo",
                filePath: row.ticket.anexoPath ?? null
              }
            : undefined
        }))}
      />
    </section>
  );
}