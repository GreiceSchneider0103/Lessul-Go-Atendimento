    ...
    <section className="page">
      ...
      {error || !ticket ? (
        ...
        <>
          ...
          <article className="card"><h2>Comentários operacionais</h2><ul>{((ticket as any).comentariosOperacionais ?? []).map((c: any)=><li key={c.id}><strong>{c.autorNome ?? "Usuário"}</strong> ({c.autorPerfil ?? "-"}) - {formatDateTimeBR(c.criadoEm)}<br/>{c.comentario}</li>)}</ul></article>
