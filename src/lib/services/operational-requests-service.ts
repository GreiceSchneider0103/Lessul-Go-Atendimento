...
export async function updateOperationalRequest(id: string, actor: AppUser, payload: Partial<{status: StatusOperacional; comentarioLoja: string; codigoRastreio: string; valorReembolso: number; valorCte: number; valorColetaEnvioPecas: number; prazoOperacional: string;}>) {
  ...
  const updated = await prisma.operationalRequest.update({ where: { id }, data: { ...payload, updatedBy: actor.id, completedAt: payload.status === "CONCLUIDA" ? new Date() : undefined } });
...
export async function listAttachments(requestId: string, actor: AppUser) {
  ...
  return prisma.operationalRequestAttachment.findMany({ where: { operationalRequestId: requestId }, orderBy: { uploadedAt: "desc" } });
}
...
export async function uploadAttachment(requestId: string, file: File, tipoAnexo: TipoAnexoOperacional, actor: AppUser) {
  ...
  if (!allowed.includes(file.type)) throw new ForbiddenError("Tipo de arquivo não permitido");
  if (file.size > 10 * 1024 * 1024) throw new ForbiddenError("Arquivo acima de 10MB");
