...
--
-- Requires 20260824170000_simplify_status_operacional to have already run
-- and committed, since it adds 'ASSISTENCIA_ENVIADA' to StatusOperacional
-- (used below) — a value added via ALTER TYPE ... ADD VALUE can't be used
...
-- OperationalRequest also carries a partial unique index whose predicate
-- embeds the enum type ('status' <> 'CONCLUIDA'); Postgres can't rewrite
-- that index across the type swap ("operator does not exist: StatusOperacional
-- <> StatusOperacional_old"), so it has to be dropped first and rebuilt
-- against the new type afterwards.
DROP INDEX "OperationalRequest_unique_active";
